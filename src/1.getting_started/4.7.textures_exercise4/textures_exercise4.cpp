...
-------------------------------
  ...
  Date:Wed Jan  3 18:03:11 CST 2018
------------------------------ 
*/
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    int visibLocation = glGetUniformLocation(ourShader.ID, "visb");
    ourShader.use();
    glUniform1f(visibLocation, VISIB_VALUE);
    glBindVertexArray(VAO);
...
void processInput(GLFWwindow *window) {
  ...
  // 当按下down表示，可见性下降; up, 可见性上升
  if ((glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)&&(VISIB_VALUE <= 1))
    VISIB_VALUE += 0.01;
  if ((glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)&&(VISIB_VALUE>= 0))
    VISIB_VALUE -= 0.01;
}
